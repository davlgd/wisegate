...
"#,
    ...
    assert!(
        response.contains("Hello from test backend"),
        "GET request failed: {}",
        response
    );
    ...
    let response = String::from_utf8_lossy(&output.stdout);
    assert!(
        response.contains("Received 10485760 bytes"),
        "Large POST request failed: {}",
        response
    );
